...
4. **翻译内容**：
   - **.md 文件**：按第 3 节规则整体翻译，生成目标 Markdown 文本。
   - **.ipynb 文件**：解析 JSON；仅重写 `cell_type == "markdown"` 单元格的 `source` 字段；其余字段原样写回；重新序列化为合法 JSON（保持 indent=1，与 Jupyter 默认格式一致）。

5. **写入译文**：将译文写入目标路径（UTF-8 编码）。
